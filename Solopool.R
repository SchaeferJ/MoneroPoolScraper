...
# General Setup
POOL_NAME <- "Solopool"
POOL_URL <- "https://xmr.solopool.org"
POOL_ID <- 3 # For Worker UIDS

# Define API-Endpoints to query
BLOCKS_ENDPOINT <- "https://xmr.solopool.org/api/blocks"
ACTIVEMINER_ENDPOINT <- "https://xmr.solopool.org/api/miners"
MINERDETAILS_ENDPOINT <- "https://xmr.solopool.org/api/accounts/"
PAYMENT_ENDPOINT <- "https://xmr.solopool.org/api/payments"
...
for(m in miners){
  ...
  minerHashes <- minerDetails[["charts"]]
...
names(workerList) <- c("Hashrate", "H1", "LastShare", "ValidShares","InvalidShares", "StaleShares", "Online", "Port","Location", "WorkerID", "Miner", "Timestamp")
...
workerList$UID <- uids
workerList$ValidShares <- NULL
workerList$InvalidShares <- NULL
workerList$StaleShares <- NULL
workerList$Online <- NULL
workerList$Port <- NULL
workerList$Location <- NULL


hashList$x <- as.POSIXct(hashList$x, origin="1970-01-01", tz="GMT")
names(hashList) <- c("RefTime","Hashrate", "Miner", "Timestamp")
